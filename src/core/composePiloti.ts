...
export function composePiloti(parameters: PilotiParameters, base: readonly ScenePiece[], upperMass: ScenePiece): MassStudy {
  ...
  const concreteMassKg = concreteVolumeMm3 / 1e9 * parameters.concreteDensityKgM3
  return { recipe: 'piloti', seed: parameters.seed, pieces, bounds, widthMm, depthMm, heightMm,
    concreteVolumeMm3, concreteDensityKgM3: parameters.concreteDensityKgM3,
    concreteMassKg, retainedCore,
    ...
    stability: analyseStability(pieces, parameters.concreteDensityKgM3, retainedCore) }
}
